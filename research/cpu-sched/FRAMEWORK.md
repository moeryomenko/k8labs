...
`research/cpu-sched/analysis/output/v08/` (clean per-family dirs, per-family CSVs,
`report-input/`, `cpu-count-compare/`; `interaction-report.md` and
`multi-cpu-validation.md` in `research/cpu-sched/analysis/output/`).
...
2. Use `cgroup-watch.sh` to observe throttling:
   ```bash
   research/cpu-sched/bin/cgroup-watch.sh <pod-name> --interval 5 --count 60
   ```
...
# See research/cpu-sched/scripts/configure-cpu-manager.sh

# Verify static policy is working
# Pods with Guaranteed QoS should show cpu pinning in cgroup
research/cpu-sched/scripts/cgroup-observe.sh <pod-name> | jq '.cpuset.cpus'
```
...
For detailed experimental results on EEVDF behavior across workload types, including vruntime trajectories, slice distributions, and deadline drift analysis, see the generated deep-dive report at `research/cpu-sched/analysis/output/DEEP-DIVE-EEVDF-EXEC.md` (regenerated by the dist pipeline).
