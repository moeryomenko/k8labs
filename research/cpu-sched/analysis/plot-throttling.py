...
def parse_aggregates(filepath):
    ...
    def parse_cell(cell):
        # Handle both '-' and ';' as separators (experiments use '-', aggregate script preserves format)
        sep = ";" if ";" in cell else "-"
        parts = cell.replace(" ", "").split(sep)
        req = ""
        lim = ""
        for p in parts:
            if p.startswith("request="):
                val = p.replace("request=", "").strip()
                if val:
                    req = val
            elif p.startswith("limit="):
                val = p.replace("limit=", "").strip()
                if val:
                    lim = val
            elif p.startswith("ls_request="):
                val = p.replace("ls_request=", "").strip()
                if val:
                    req = val
            elif p.startswith("ls_limit="):
                val = p.replace("ls_limit=", "").strip()
                if val:
                    lim = val
...
def plot_throttled_time_vs_ratio(df, output_dir):
    ...
    ax.scatter(
        subset["request_limit_ratio"],
        subset["mean_throttled_time_ratio"],
        c="steelblue",
        s=80,
        alpha=0.7,
    )
...
def plot_usage_vs_limit(df, output_dir):
    ...
    for req_val in sorted(df["request_m"].unique()):
        ...
        label = f"request={req_val}m" if req_val > 0 else "no request"
        ax1.bar(
            subset["limit_m"] + (req_val / 100),
            subset["mean_usage_usec"] / 1000,
            width=80,
            label=label,
            alpha=0.6,
        )
...
def main():
    if len(sys.argv) < 2:
        print(
            "No data file provided. Usage: python3 plot-throttling.py <aggregates.csv> [--output-dir path]"
        )
        sys.exit(1)
